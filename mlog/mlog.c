...
void writeLog( int loglvl, const char* str, ... ) {
    ...
    if( loglvl < MLOG_INFO ){
        ...
        } else if( loglvl == MLOG_WARN && dbgLevel >= MLOG_WARN ) {
            outColor = COL_WARN;
            sprintf( msg, "%s\tWARN  : ", date );      // 1: Warning
        } else if( loglvl == MLOG_DEBUG && dbgLevel >= MLOG_DEBUG ) {
            outColor = COL_DEBUG;
            sprintf( msg, "%s\tDEBUG : ", date );      // 2: Debug
        } else if( loglvl == MLOG_VERBOSE && dbgLevel >= MLOG_VERBOSE ) {
            outColor = COL_VERBOSE;
            sprintf( msg, "%s\tDEBUG : ", date );      // 3: Verbose
        } else if( loglvl == MLOG_LOGGER && dbgLevel >= MLOG_DEBUG ) {
            outColor = COL_LOGGER;
...
void setLogDebugLevel( int level ) {
    ...
    if( level >= MLOG_INFO && level <= MLOG_VERBOSE ) {
         dbgLevel = level;
...
void setLogFile( const char* file ) {
    ...
    strcpy( logFile, file );
    writeLog( MLOG_LOGGER, "Log file set to '%s'", logFile );
}
...
void flushLog() {
    ...
    writeLog( MLOG_LOGGER, "Log file '%s' cleared", logFile );
}
...
void loadConfig( const char* config ) {
    ...
    if( fd == -1 ) {
        // Write error message
        writeLog( MLOG_LOGGER, "Unable to open config file: '%s'", config );
...
static char** getPrettyBacktrace( void* addresses[], int array_size ) 
{
    ...
    for( i = 0; i < array_size; i++ ) {
        ...
        if( line == NULL ) {
            writeLog( MLOG_LOGGER, "Failed to execute command: '%s'. Defaulting to standard backtrace.", command );
            for( i = 0; i < array_size; i ++ ) {
                free( backtrace_strings[i] );
            }
            free( backtrace_strings );
        ...
        if( fgets( backtrace_strings[i] , 255, line ) == NULL ) {
            writeLog( MLOG_LOGGER, "Failed to get pretty backtrace strings. Defaulting to standard backtrace." );
            for( i = 0; i < array_size; i ++ ) {
                free( backtrace_strings[i] );
            }
            free( backtrace_strings );
    ...
    if( !address_evaluation_successful ) {
        writeLog( MLOG_LOGGER, "Command '%s' failed to evaluate addresses. Defaulting to standard backtrace.", command );
        for( i = 0; i < array_size; i ++ ) {
            free( backtrace_strings[i] );
        }
        free( backtrace_strings );
...
mlog_namespace const mlog = { writeLog, writeStackTrace, setLogDebugLevel, setLogFile, setLogSilentMode, setLineWrap, flushLog, loadConfig };
